...
def _apply_calendar_and_hours(df_wide: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1) Business-day filter
    cal = _get_ql_calendar()
    ts = pd.to_datetime(df_wide["ts"])
    if cal is not None:
        try:
            # Vectorized: build a mask using QuantLib businessDay
            import QuantLib as ql
            # Convert to ql.Date (YYYY, M, D)
            ymd = np.array([(t.year, t.month, t.day) for t in ts], dtype=int)
            # Build a per-day mask, then map back to timestamps
            days = pd.to_datetime(
                { "year": ymd[:,0], "month": ymd[:,1], "day": ymd[:,2] }
            ).drop_duplicates().sort_values()
            bd_mask = []
    ...
    start_str, end_str = TRADING_HOURS
    pre = len(df_wide)
    # make tz-aware in local tz for time slicing
    df_wide = df_wide.copy()
    df_wide["ts_local"] = df_wide["ts"].dt.tz_localize("UTC").dt.tz_convert(tz_local)
    # Use between_time on DatetimeIndex
    temp = df_wide.set_index("ts_local").sort_index()
    temp = temp.between_time(start_str, end_str)
    # back to UTC-naive ts
    temp["ts"] = temp.index.tz_convert("UTC").tz_localize(None)
    temp = temp.drop(columns=["ts_local"])
    kept = len(temp)
    if pre > 0:
        print(f"[CAL] kept {kept:,}/{pre:,} rows ({(kept/pre*100):.2f}%) after calendar+hours")
...
def _pca_apply_block(df_block: pd.DataFrame, pca_model: dict) -> pd.Series:
    ...
    w = df_block.pivot(index=None, columns="tenor_yrs", values="rate")
    if any(c not in w.columns for c in cols):
        return out

    x  = w[cols].iloc[0].values.astype(float)
    xc = x - mu
    score = comps @ xc
    recon = comps.T @ score
    z = recon
