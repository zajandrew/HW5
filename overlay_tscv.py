...
Metric: Winsorized Daily t-Statistic (Balances Daily Consistency & Tail Risk).
"""
...
# We want to find parameters that make money "Naked" first.
LOCKED_FILTERS = {
...
def load_global_data():
    ...
    # Ensure REGIME_WINDOW is respected in config before this runs
    signals = hf.get_or_build_hybrid_signals(force_rebuild=False)
    
    # 2. Define Month List from Enhanced Files
...
def _worker_task(task_tuple):
    ...
    Aggregates trades by day, clips the worst 5% of days (simulating filter),
    and returns the t-stat of that cleaned daily series.
    """
    ...
    # Load the specific synthetic tape
    ...
    # Dummy Filters (Objects needed for signature, but thresholds are -99)
    ...
    try:
        ...
        if pos.empty:
            ...
            # 1. Aggregate to Daily PnL (Winsorize Days, not Trades)
            # Use close_ts to capture Realized outcomes
            pos["day"] = pos["close_ts"].dt.floor("D")
            daily_series = pos.groupby("day")["pnl_net_bp"].sum()
            daily_vals = daily_series.values
            
            if len(daily_vals) < 10: # Need minimal days for t-stat
                daily_t_stat = -10.0
                avg_daily = daily_vals.mean() if len(daily_vals) > 0 else 0.0
            else:
                # 2. Winsorize Left Tail (5%)
                # Simulates "Filter would have stopped the crash day"
                floor_val = np.percentile(daily_vals, 5)
                
                # Safety: If 5th percentile is positive (great strategy), don't clip upwards
                # We only clip if floor_val is negative (a loss).
                # Actually, standard winsorization clips regardless, but let's be conservative.
                # Ideally we clip to the max of (floor, some_stop_loss).
                # Let's just stick to raw percentile clipping.
                clipped_vals = np.clip(daily_vals, a_min=floor_val, a_max=None)
                
                # 3. Calculate t-Stat on Daily Series
                # t = (Mean / Std) * sqrt(N_Days)
    ...
    result["daily_t_stat"] = daily_t_stat # <--- Primary Metric
...
def run_tournament():
    # 1. Load Global Signals
    _, signals, months = load_global_data()
    
    # 2. Identify Synthetic Tapes
    ...
    # --- ROUND 1: THE CULLING (Tape 0) ---
    # Goal: Quickly kill strategies that don't work at all.
    ...
    # Sort by Winsorized Daily t-Stat
    df_r1 = df_r1.sort_values("daily_t_stat", ascending=False)
    ...
    # --- ROUND 2: ROBUSTNESS (Tapes 1-4) ---
    # Goal: Test consistency across different time-shifts.
    ...
    # Aggregate: Mean t-Stat across tapes
    ...
    # --- ROUND 3: VALIDATION (All Tapes) ---
    # Goal: Final verification on full distribution.
    ...
    final_stats = df_r3.groupby(param_cols).agg(
        ...
        min_t_stat=("daily_t_stat", "min"), # Robustness check
