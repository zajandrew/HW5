...
def _pca_apply_block(df_block: pd.DataFrame, pca_model: dict) -> pd.Series:
    ...
    # Use HEAD(1) for scoring consistency
...
def build_month(yymm: str) -> None:
    ...
    # We have the raw data loaded now. Save the summary cache so next month doesn't reload it.
    cache_name = f"{yymm}_summary_{decision_freq}_OPEN.parquet"
    cache_path = path_data / cache_name

    if not cache_path.exists() and not df_wide.empty:
        try:
            # Same logic as load_history: Head(1) for Open
            ...
            print(f"[WARN] Failed to generate self-summary: {e}")
    ...
    # 3. Create Panel
    df_target_snaps = df_long.sort_values("ts").groupby(["decision_ts", "tenor_yrs"], as_index=False).head(1)
    df_context = pd.concat([df_history, df_target_snaps]).sort_values("ts").reset_index(drop=True)
    ...
    def _one(dts):
        ...
        # HEAD(1) for Snapshot
        out = snap.sort_values("ts").groupby("tenor_yrs", as_index=False).head(1).reset_index(drop=True)
        ...
        if pca_enable:
            ...
            if len(cols_now) >= pca_n_comps:
                ...
                panel = df_context[(df_context["ts"]>=t_start) & (df_context["ts"]<t_end) & 
                                   (df_context["tenor_yrs"].isin(cols_now))]
